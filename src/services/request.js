...
export const getDetailsCoinData = async(coinId) => {
    try {
...
export const getCoinMarketData = async(pageNumber = 1) => {
    try {
...
export const getWatchlistedCoins = async(pageNumber = 1, coinIds) => {
    try {
        const response = await axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${coinIds}&order=market_cap_desc&per_page=${pageNumber}&page=1&sparkline=false&price_change_percentage=24h`)
        return response.data;
    } catch (e) {
        console.log(e);
    }
